...
def write_file_use(t=""):
    """Use write_file."""
    f = cl_inp(('Name of the output file') + ' : ')
    if t == "":
        t = cl_inp('Text : ')
    write_file(f, t)
...
def ask_text():
    ...
    f = inp_lst(('Read text from file ?') + ' ' + ('(y/n)') + ' : ', (('y'), ('n'), ('yes'), ('no')))
    if f in (('y'), ('n'), ('yes'), ('no')):
        t = read_file_use()
    else:
...
def give_result(res):
    ...
    r = inp_lst(('Write the result in a file ?') + ' ' + ('(y/n)') + ' : ', (('y'), ('n'), ('yes'), ('no')))
    if r in (('y'), ('n'), ('yes'), ('no')):
        write_file_use(res)
    else:
        print(('Result') + ' :\n')
...
def make_ciph(ciph, key=None, key2=None, alf=alf_az, ignore=False, verbose=True, interface=None, **kargs):
    ...
    if ciph in ciph_types['verbose, interface']:
        ...
        raise NotImplemented(('The cipher "{}" was NOT found !!!').format(ciph) + '\n' + ('If it exists, please add it to the dict "ciph_types".'))
...
class FreqAna:
    ...
    def analyse(self, txt, prec=1, verbose=True):
        ...
        if verbose:
            ...
            ret += '\n' + ('Number of total occurences : {}').format(occ) + '\n\n'
...
crypta_ana = {
    ('Frequence analysis'): freqana_str,
